...
test('show Output Format switcher with correct title', () => {
  render(
    <OutputFormat
      format={mockFormat}
  ...
  expect(screen.getByText(localize(Localization.OUTPUT_FORMAT))).toBeInTheDocument()
})

test('show all predefined output formats', () => {
  render(
    <OutputFormat
      format={mockFormat}
      updateProfile={jest.fn()}
