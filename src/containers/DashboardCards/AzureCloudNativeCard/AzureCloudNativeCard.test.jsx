...
test('should render correct layout with AI-Prompted document types', () => {
  render(<AzureCloudNativeCard {...defaultProps} />)

  expect(screen.getByText(localize(Localization.AZURE_CLOUD_NATIVE))).toBeInTheDocument()
...
test('should dispatch goTo action with correct filters when card is clicked', async () => {
  ...
  const cardTitle = screen.getByText(localize(Localization.AZURE_CLOUD_NATIVE))
  ...
  expect(goTo).toHaveBeenCalledWith(
    ...
    expect.objectContaining({
      filters: expect.objectContaining({
        extractionType: EXTRACTION_TYPE_FILTER_KEY.azureCloudExtractor,
      }),
    }),
  )
})
